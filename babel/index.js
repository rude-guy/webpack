/**
 * babel插件
 * 主要还是 @babel/core中的transform、parse对于ast的处理
...
// babel核心转化库 包括 core => AST => code的转化现实

/**
 * babel/core 其实就可以相当于 esprima + Estraverse + Escodegen
 * 它会将原有的sourceCode转化为AST语法树
 * 遍历老的语法树
 * 遍历老的语法树时候 会检查传入的插件/或者是第三个参数传入的「visitor」
 * 修改对应匹配的节点
 * 生成新的语法树
 * 之后生成新的代码
 */
...
// babel/types 工具库 该模块包含了手动构建TS的方法，并检查AST节点的类型。(根据不同节点类型进行转化实现)
const babelTypes = require('@babel/types');

// 转化箭头函数的插件
// const arrowFunction = require('@babel/plugin-transform-arrow-functions');
const arrowFunction = require('./plugin-transform-arrow-functions');

const sourceCode = `const arrowFn = () => { console.log(this) };`;
